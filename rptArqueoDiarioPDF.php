...
$logoEmpresa=$datLogoTiendaRopa[0];
...
<img src='imagenes/$logoEmpresa' width='80'></td>
...
$sql="select s.`fecha`,  
	...
	s.hora_salida,s.cod_chofer,s.cod_salida_almacenes,s.salida_anulada,s.monto_cancelado_usd,s.tipo_cambio, s.monto_total, s.descuento, s.siat_usuario
...
$sqlTarjetas="select s.`fecha`,  
	...
	s.hora_salida,s.cod_chofer,s.cod_salida_almacenes,s.monto_cancelado_usd,s.tipo_cambio, s.monto_total, s.descuento,
 	0 as nombre_banco, (SELECT nro_tarjeta FROM tarjetas_salidas where cod_salida_almacen=s.cod_salida_almacenes limit 1)numero_tarjeta, s.siat_usuario
...
while($datos=mysqli_fetch_array($resp)){
	...
	$horaVenta=$datos[9];
	$personalCliente=$datos['siat_usuario'];
...
while($datos=mysqli_fetch_array($respTarjeta)){
	...
	<td>$nombreTipoPago</td>
	<td>$datosDoc</td>
	<td align='right'><small><small><small><small>$tarjetaNumero</small></small></small></small></td>
	<td align='right'>$montoVentaBrutoFormat</td>
	<td align='right'>$descuentoVentaCabFormat</td>
		<td align='right'>$descuentoCabPorcentaje</td>
