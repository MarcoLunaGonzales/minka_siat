...
<div  style="height: 49.4%">
        ...
        <table class="table2">
            <tr>
                <td width="20%" class="text-center">MÉDICO</td>
                <td width="28%" class="text-center">DETALLE</td>
            ...
            <?php
                ...
                // Inicializamos un array para estructurar los datos
                $data = [];

                while ($row = mysqli_fetch_assoc($respDetalle)) {
                    $especialidad = $row['especialidad'];
                    $medico = $row['nombreApellidoMedico'];

                    // Buscar si ya existe esta combinación de codInterno, especialidad y medico
                    $key = array_search("$especialidad-$medico", array_column($data, 'uniqueKey'));

                    if ($key === false) {
                        // Si no existe, agregar una nueva entrada
                        $data[] = [
                            'nroQuirofanoSalaOperaciones' => $row['nroQuirofanoSalaOperaciones'],
                            'especialidad' => $especialidad,
                            'medico' => $medico,
                            'especialidadMedico' => $row['especialidadMedico'],
                            'nroFacturaMedico'   => $row['nroFacturaMedico'],
                            'nitDocumentoMedico' => $row['nitDocumentoMedico'],
                            'nroMatriculaMedico' => $row['nroMatriculaMedico'],
                            'detalle' => [],
                            'uniqueKey' => "$especialidad-$medico" // Solo para búsqueda, se removerá más adelante
                        ];

                        $key = count($data) - 1;
                    }

                    // Agregar el detalle al médico correspondiente
                    $data[$key]['detalle'][] = [
                        'codInterno' => $row['cod_material'],
                        'descripcion_material' => $row['descripcion_material'],
                        'precio_unitario' => $row['precio_unitario'],
                        'cantidad_unitaria' => $row['cantidad_unitario'],
                ...
                // Remover las claves únicas usadas para búsqueda
                foreach ($data as &$entry) {
                    unset($entry['uniqueKey']);
                }
                
                foreach ($data as $detalle) {
                    $descripcion = $detalle['especialidad'];
                    $medico = $detalle['medico'];
                    $nitDocumentoMedico = $detalle['nitDocumentoMedico'];
                    $nroFacturaMedico = $detalle['nroFacturaMedico'] ?? 'N/A'; // Manejo de valor opcional
                    $especialidadMedico = $detalle['especialidadMedico'] ?? 'N/A'; // Manejo de valor opcional
                
                    echo "<tr>
                        <td>
                            <b>Quirofano/Gabinete:</b> {$detalle['nroQuirofanoSalaOperaciones']}<br>
                            <b>Médico:</b> {$medico} <br>
                            <b>Factura:</b> {$nroFacturaMedico} <b>Nit:</b> {$nitDocumentoMedico}
                        </td>";
                    ...
                    echo "<td>";
                    echo "<b>Especialidad: </b>{$descripcion}<br>";
                    echo "<ul class='list-unstyled' style='margin: 0; padding: 10px;'>"; // Para una mejor presentación
                    foreach ($detalle['detalle'] as $servicio) {
                        $codInterno = $servicio['codInterno'];
                        $precioUnit = (float)$servicio['precio_unitario'];
                        $cantUnit   = $servicio['cantidad_unitaria'] ?? 0;
                        $descUnit   = (float)$servicio['descuento_unitario'];
                        $montoUnit  = number_format(($cantUnit * $precioUnit) - $descUnit, 2, '.', ',');
                        $descripcionServicio = htmlspecialchars($servicio['especialidadDetalle'] ?? 'Sin descripción');
                
                        $totalGrupo += (float) $montoUnit;
                        $cantidadTotal += (float) $cantUnit;
                        $descuentoTotal += (float) $descUnit;
                
                        echo "<li>[{$montoUnit} Bs.] - {$descripcionServicio}</li>";
                    }
                    echo "</ul>";
                    echo "</td>";
                
                    echo "<td class='text-center'>$unidad_medida</td>
                        <td class='text-center'>1</td>
                        <td class='text-right'>" . number_format($totalGrupo, 2, '.', ',') ."</td>
                        <td class='text-right'>{$descuentoTotal}</td>
                        <td class='text-right'>" . number_format($totalGrupo, 2, '.', ',') ."</td>
                    </tr>";
                    $montoTotal=$montoTotal+$totalGrupo;
                }
