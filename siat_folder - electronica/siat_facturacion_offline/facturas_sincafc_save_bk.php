...
echo "<script language='Javascript'>
  ...
  $descripcionX="";//para el tipo de evento
  $codigoPuntoVenta=0; 
  $flagSuccess=false;
  $error=true;
...
if($DatosConexion[0]==1){
    ...
    if($cufd<>"0"){
      ...
      if($codigoEvento<>-1){
       ...
       if($sw==0){
        ...
        if($codigo==1){
          $error=false;
          // $descripcionError="<b>Paquete: <br>Paso 1.-</b> ".$descripcionPaquete."<br><b>Paso 2.-</b> ".$descripcionValidacion;
          $descripcionPaquete=" <span style=\"color:green;\">".$descripcionPaquete."</span>";
          // $descripcionValidacion=" <span style=\"color:green;\">".$descripcionValidacion."</span>";
  ...
  // }else{
    ...
    Swal.fire({
      title: 'EL PROCESO TERMINÓ CON EL SIGUIENTE DETALLE:',
      html: '<table style=\"border:1px;font-size:14px\"><tr><td>".$descripcioNuevo."</td></tr></table>',
      type: 'success'
    }).then(function() {
        ".$url_retorno."
    });
